...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS swift_transfers (
    ...
    -- Swift HKAG tracks the intercompany wire Hong Kong sends Ningbo — has
    -- NO relation to Supplier Contracts/Supplier Flow (that's HKAG/Ningbo
    -- paying the factory, a separate leg entirely). Generated by an
    -- explicit "Generate Swift Transfer" button on the Proforma's own
    -- Ningbo -> HKAG Internal Proforma popup (see NingboInternalForm on the
    -- frontend and POST /api/proformas/:id/generate-swift below), using
    -- that popup's own Item Values total/currency and Payment Terms — one
    -- Swift row per Proforma, re-clicking the button updates this same row
    -- (amount/currency/notes) instead of creating a duplicate.
    ...
    -- Invoice's "HKAG 084", or now the Proforma's own number) — easy to
    ...
    currency TEXT DEFAULT 'USD',
    status TEXT DEFAULT 'Pending',
    -- Still available for manually splitting a transfer into Deposit/
    -- Balance (edited directly on the Swift HKAG screen, same UI as
    -- Supplier Flow's own schedule fields) — no longer auto-derived from
    -- anything, since the Internal Proforma's own Payment Terms is just
    -- freeform text with no fixed schedule shape to parse.
...
const migrations = [
  ...
  // Each installment gets its own Payment Notice (see the
  // payment-notice-xlsx route's ?pct=&label= params).
  ...
  // Same "Partial" tracking Supplier Flow/Swift HKAG already have —
  // paid_amount only matters when status is "Partial" (see the new PATCH
  // /api/commercial-invoices/:id/status route), full total is implied for
  // "Paid" and 0 otherwise.
  ['commercial_invoices', 'paid_amount', 'REAL DEFAULT 0'],
  ['commercial_invoices', 'paid_date', 'TEXT'],
  ...
  // 'contract_id' below is a leftover column from an earlier design that
  // tied Swift HKAG to Supplier Contracts — kept (never dropped, SQLite
  // migrations here are additive-only) but no longer written to; Swift HKAG
  // ties to the Proforma's own "Generate Swift Transfer" button instead
  // (see 'proforma_id' and POST /api/proformas/:id/generate-swift).
  ['swift_transfers', 'contract_id', 'INTEGER'],
  ['swift_transfers', 'proforma_id', 'INTEGER'],
  ...
  // Supporting files (loading photos, customs docs, etc.) — same JSON array
  // of { url, name } shape used by Inspections/Samples/Swift HKAG.
  ['packing_lists', 'media', 'TEXT'],
